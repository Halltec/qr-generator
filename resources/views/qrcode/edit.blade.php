...
<section class="w-full">
		...
		<form method="POST" action="{{ route('qrcode.update', $qrcode) }}" class="my-6 w-full space-y-6">
			...
			<div>
				...
				<textarea id="description" name="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 bg-white dark:bg-greay-900 text-gray-900 dark:text-gray-100 shadow-sm focus:border-gray-400 focus:ring-0" placeholder="Descripción opcional">{{ old('description', $qrcode->description) }}</textarea>
				@error('description')
